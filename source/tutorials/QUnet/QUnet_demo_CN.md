# 基于VQNET和Pyqpanda的QUnet网络

## 概述

我们探索如何将经典神经网络部分量化，以创建混合量子经典QUnet神经网络。我们将编写一个将**Pyqpanda**与**VQNET**集成的简单示例。代码可见[QUnet_test.py](../../pyVQNet/examples/qunet_test.py)。

## 流程

### 数据准备

我们从VOCdevkit/VOC2012官方库中获取数据。任务输入：VOC2012输入图像及其对应图像标签。例如：

<img src=".\2007_000121.jpg" alt="2007_000121" style="zoom:50%;" />           <img src=".\2007_000121.png" alt="2007_000121" style="zoom:50%;" />

任务输出：保存QUnet的训练模型。对其他VOC中的图像进行分割，输出分割结果。
...
首先利用混合量子经典神经网络框架构建网络模型，然后利用模型对数据进行训练并将训练后的模型保存为本地文件，最后利用模型加载器对新的输入图片进行预测。
...
使用Adam完成此任务就足够了，model.parameters（）是需要计算的参数。
