# Qunet network based on VQNet and Pyqpanda

## Overview

We explore how to partially quantify the classical neural network to create a hybrid quantum classical qunet neural network. We will write a simple example of integrating  **pyqpanda**  with  **VQNet** . The code can be seen in [QUnet_test.py](../../pyVQNet/examples/qunet_test.py)。

## Pipeline

### Data preparation

We obtain data from the official vocdevkit / voc2012 database. Task input: voc2012 input image and its corresponding image label. For example:

<img src="F:/vqnet/vqnet/tutorials/QUnet/2007_000121.jpg" alt="2007_000121" style="zoom:50%;" />           <img src="F:/vqnet/vqnet/tutorials/QUnet/2007_000121.png" alt="2007_000121" style="zoom:50%;" />

Task output: save qunet training model. Segment the images in other VOCs and output the segmentation results.
...
```python
          ┌────────────┐ ┌────────────┐                      ┌────┐ 
q_0:  |0>─┤RY(0.903385)├ ┤RZ(1.015064)├ ───■── ────── ────── ┤CNOT├ 
          ├────────────┤ ├────────────┤ ┌──┴─┐               └──┬─┘ 
q_1:  |0>─┤RY(0.910402)├ ┤RZ(1.027939)├ ┤CNOT├ ───■── ────── ───┼── 
          ├────────────┤ ├────────────┤ └────┘ ┌──┴─┐           │   
q_2:  |0>─┤RY(0.908077)├ ┤RZ(1.023687)├ ────── ┤CNOT├ ───■── ───┼── 
          ├────────────┤ ├────────────┤        └────┘ ┌──┴─┐    │   
q_3:  |0>─┤RY(0.908077)├ ┤RZ(1.023687)├ ────── ────── ┤CNOT├ ───■── 
          └────────────┘ └────────────┘               └────┘        
...
```python
from unet_quantum_qconv import QCNN_, quanconv_
import numpy as np
import cv2
import os
...
class PreprocessingData:
    ...
    def processing(self):
        ...
        list1 = os.listdir((self.path+"/train"))
        for i in range(len(list1)):
            temp_data = cv2.imread(self.path+"/train" + '/' + list1[i], cv2.IMREAD_COLOR)
            ...
            label_data = cv2.imread(self.path+"/label" + '/' +list1[i].split(".")[0] + ".png", cv2.IMREAD_COLOR)
...
### Constructing hybrid quantum neural network

Firstly, the hybrid quantum classical neural network framework is used to construct the network model, then the model is used to train the data, and the trained model is saved as a local file. Finally, the model loader is used to predict the new input picture.
...
from pyvqnet.nn.loss import BinaryCrossEntropy, MeanSquaredError
...
It is sufficient to use Adam to complete this task. Model. Parameters () is the parameter to be calculated.
...
BinaryCrossEntropy() is loss function
...
```python
 if __name__ == "__main__":
    ...
    for epoch in range(1, epochs):
        ...
        for i, (x, y) in enumerate(trainset):
            ...
            if i == 1:
                ...
                plt.savefig("./result/" + str(epoch) + "_" + str(i) + ".jpg")
        ...
        if SAVE_FLAG:
            ...
            save_parameters(model.state_dict(), "Q-Unet_End.model")
            SAVE_FLAG = False
        else:
    ...
    plt.figure()
    lines3 = out_read.readlines()
    data3 = []
    for line3 in lines3:
        floatstr3 = float(line3)
        data3.append(floatstr3)
    out_read.close()
    plt.plot(data3)
...
![image-20211108163935171](./Qunet_training_loss.png)

Visualized results:

<img src=".\6_1.jpg" alt="1_1" style="zoom:80%;" />

<img src=".\1_1.jpg" alt="1_1" style="zoom:80%;" />

<img src=".\10_1.jpg" alt="10_1" style="zoom:80%;" />